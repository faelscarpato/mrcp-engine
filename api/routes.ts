...
export const routeHandlers: Record<string, (req: any, res: any) => Promise<any>> = {
  ...
  "/api/report": async (req, res) => {
    ...
    if (fs.existsSync(rootJsonPath)) {
      try { analysisData = JSON.parse(fs.readFileSync(rootJsonPath, "utf-8")); } catch {}
    }
    if (!analysisData) {
  ...
  "/api/export-report": async (req, res) => routeHandlers["/api/report"](req, res),
  ...
  "/api/instructions": async (req, res) => routeHandlers["/api/guidelines"](req, res),
  ...
  "/api/document-analyzer": async (req, res) => {
    ...
    const filterExtensions = req.query.ext ? String(req.query.ext).split(",") : req.body?.filterExtensions;
    const maxFiles = req.query.maxFiles ? parseInt(String(req.query.maxFiles), 10) : req.body?.maxFiles;
    const result = await analyzeDocumentRepository({ repoUrl, filterExtensions, maxFiles, githubToken: process.env.GITHUB_TOKEN });
    return sendFormattedResponse(req, res, "document_analyzer", repoUrl, { status: "success", document_analysis: result });
  },
  "/api/document-analysis": async (req, res) => routeHandlers["/api/document-analyzer"](req, res),
  "/api/doc-analyzer": async (req, res) => routeHandlers["/api/document-analyzer"](req, res),
  ...
  "/api/full-suite": async (req, res) => {
    ...
    if (!repoUrl) return res.status(400).json({ status: "error", error_code: "MISSING_TARGET_URL" });
    const { runFullRepositoryDiagnostic } = await import("../packages/core/lib/analysis/full-suite.js");
    const result = await runFullRepositoryDiagnostic({ repoUrl, taskContext: req.query.task || req.body?.task, generateStubs: req.query.stubs !== "false" });
    return sendFormattedResponse(req, res, "full_repository_diagnostic_suite", repoUrl, { status: "success", full_diagnostic: result });
  },
  "/api/full-analysis": async (req, res) => routeHandlers["/api/full-suite"](req, res),
  "/api/deep-analysis": async (req, res) => routeHandlers["/api/full-suite"](req, res)
};
