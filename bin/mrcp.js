...
`;
  ...
  const skillPaths = [
    ...
    join(home, '.gemini', 'antigravity-cli', 'custom', 'skills', 'mrcp-engine', 'SKILL.md'),
  ...
  for (const sPath of skillPaths) {
    try {
      ...
      const contentToWrite = sPath.endsWith('SKILL.md') || sPath.endsWith('.mdc') ? (skillFrontmatter + ruleContent) : ruleContent;
  ...
  console.log(`   ✨ Regras e Skills injetadas com sucesso em ${skillsInjected} locais (Cursor, Claude, Antigravity, Windsurf, Copilot)!`);
}
