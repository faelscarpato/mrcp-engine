...
1. **⚡ Modo Modular (Sob Demanda):** A IA ou usuário chama exatamente a ferramenta que precisa (ex: apenas `mrcp_api_contract_generator` ou apenas `mrcp_security_compliance_audit`), economizando tempo e focando em uma tarefa isolada.
2. **🚀 Modo Diagnóstico Completo (All-in-One Pipeline - `GET /api/full-analysis` ou `mrcp_run_full_repository_suite`):** A IA ou usuário dispara **uma única chamada** que executa automaticamente as 12 ferramentas de análise de código em fila sequencial. Cada relatório intermediário é gravado em tempo real em `mrcp-analysis.json` e um painel executivo consolidado é retornado. **Elimina a necessidade de pedir autorização ao usuário a cada ferramenta.**

---

## 🛠️ Lista Completa das 25 Ferramentas MCP / REST Endpoints

O MRCP-Engine expõe um total de **24 ferramentas MCP** e rotas REST serverless divididas em 5 categorias principais:
