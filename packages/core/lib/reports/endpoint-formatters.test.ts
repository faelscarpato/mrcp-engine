...
describe('formatEndpointToMarkdown', () => {
  const dummyHeader = { summary: { estimatedTokensWithoutMrcp: 100, estimatedTokensWithMrcp: 50, tokenSavingsPercent: 50 } };
  ...
  it('should format security_compliance_audit gracefully', () => {
    const data = {
      ...
      security_audit: {
        ...
        vulnerabilities: [
          { id: 'SEC-01', severity: 'CRITICAL', category: 'Auth', file: 'auth.ts', line: 10, description: 'Token ex', remediationSnippet: 'Fix' }
        ]
      }
    };
    const result = formatEndpointToMarkdown('security_compliance_audit', 'repo', data);
  ...
  it('should format dead_code_pruner correctly', () => {
    const data = {
      ...
      dead_code_analysis: {
        ...
        deadSymbols: [{ name: 'Test', file: 'test.ts', line: 1, kind: 'function' }]
      }
    };
