...
async function testAllNewEndpoints() {
  console.log("==================================================================");
  console.log("🧪 INICIANDO TESTES DOS 5 NOVOS ENDPOINTS DO MRCP ENGINE (v2.3.0)");
  console.log("==================================================================");
  ...
  try {
    ...
    test("1. API Contract Generator: detecta rotas e gera OpenAPI 3.0", apiRes.isApplicable && apiRes.totalRoutes > 0 && apiRes.openapiSpec.openapi === "3.0.3", {
      totalRoutes: apiRes.totalRoutes,
      frameworks: apiRes.frameworksDetected,
      routesSample: apiRes.routes.slice(0, 3)
    });
    test("1. API Contract Generator: gera SDK TypeScript tipado", Boolean(apiRes.typescriptSdkSnippet.includes("class ApiClient")), {
  ...
  try {
    ...
    test("2. Code Health Scorer: calcula Maintainability Index e Nota", healthRes.isApplicable && healthRes.maintainabilityIndex > 0 && ["A", "B", "C", "D", "F"].includes(healthRes.letterGrade), {
    ...
    test("2. Code Health Scorer: identifica prioridades de refatoração", Array.isArray(healthRes.topRefactoringPriorities), {
  ...
  try {
    ...
    test("3. Env Validator: mapeia variáveis de ambiente e gera Zod schema", envRes.isApplicable && envRes.totalVariablesDetected > 0 && envRes.zodSchemaSnippet.includes("z.object"), {
  ...
  try {
    const monoRes = await analyzeMonorepoGraph({ repoUrl: localRepo, changedFiles: ["packages/core/lib/cache.ts"] });
    test("4. Monorepo Graph: mapeia workspaces, dependências e ordem de build", monoRes.isMonorepo && monoRes.packagesCount > 0 && monoRes.topologicalBuildOrder.length > 0, {
  ...
  try {
    const docRes = await generateDocumentation({ repoUrl: localRepo, targetFilePath: "packages/core/lib/analysis/code-health.ts" });
    test("5. Doc Generator: extrai símbolos sem doc e gera TSDoc", docRes.isApplicable && docRes.totalUndocumentedSymbols > 0 && docRes.markdownApiReference.includes("API Reference"), {
  ...
  console.log("==================================================================");
  console.log(`📊 RESULTADO FINAL: ${passCount} PASSOU / ${failCount} FALHOU (Total: ${passCount + failCount})`);
  console.log("==================================================================");
