...
function extractCalleeName(node: TreeSitterNode): { name: string; isMethod: boolean } | null {
  if (node.type === "call_expression" || node.type === "new_expression" || node.type === "call") {
    const fn = getField(node, "function") ?? (node.namedChildren && node.namedChildren.length > 0 ? node.namedChildren[0] : null);
    if (fn) {
      ...
      if (fn.type === "member_expression" || fn.type === "attribute") {
        const prop = getField(fn, "property") ?? getField(fn, "attribute") ?? (fn.namedChildren && fn.namedChildren.length > 1 ? fn.namedChildren[1] : null);
...
function collectCallExpressions(root: TreeSitterNode, nodeTypes: string[]): CallExpression[] {
