...
async function runVerification() {
  console.log("=== INICIANDO VALIDAÇÃO DAS NOVAS LÓGICAS DETERMINÍSTICAS DO MRCP ENGINE ===");
  ...
  try {
    const reactRes = await resolveDependencyCompatibility({ packageName: 'react', targetVersion: 'latest' });
    assert(
      reactRes.isApplicable === true && reactRes.resolvedVersion === '19.2.8' && reactRes.safeInstallCommand.includes('19.2.8'),
      'Dependency Resolver: Real NPM Registry fetch for react (resolved to 19.2.8)',
      reactRes
    );

    const nonExistent = await resolveDependencyCompatibility({ packageName: 'non-existent-lib-fake-123456', targetVersion: 'latest' });
    assert(
      nonExistent.isApplicable === false && Boolean(nonExistent.message?.includes('Não se aplica')),
  ...
  try {
    ...
    assert(
      ...
      Boolean(noSchemaRes.message?.includes('Não se aplica a esse repositório')) &&
  ...
  try {
    ...
    assert(
      nonApplicableJs.isApplicable === false && Boolean(nonApplicableJs.message?.includes('Não se aplica')),
...
+  const x = 1;`;
    ...
    assert(
      validSummarizer.isApplicable === true && validSummarizer.domainChanges.length > 0,
      'Git Diff Summarizer: Valid diff parsed with domain and modified functions',
      validSummarizer
    );

    const invalidSummarizer = await summarizeGitDiff({ diffContent: 'not a git diff' });
    assert(
      invalidSummarizer.isApplicable === false && Boolean(invalidSummarizer.message?.includes('Não se aplica')),
  ...
  try {
    const contracts = processRepositoryHotspots([
      { id: 'file:script.js', label: 'script.js', path: 'script.js', language: 'JavaScript', complexity: 24, degree: 0 }
    ]);
    assert(
      contracts.length === 1 &&
