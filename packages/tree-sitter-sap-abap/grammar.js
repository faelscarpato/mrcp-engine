module.exports = grammar({
  ...
  rules: {
    ...
    report_statement: ($) =>
      seq(kw("report"), field("name", $.identifier), "."),
    program_statement: ($) =>
      seq(kw("program"), field("name", $.identifier), "."),
    ...
    _class_section: ($) =>
      choice($.public_section, $.protected_section, $.private_section),

    public_section: ($) =>
      seq(kw("public"), kw("section"), ".", repeat($._section_statement)),
    protected_section: ($) =>
      seq(kw("protected"), kw("section"), ".", repeat($._section_statement)),
    private_section: ($) =>
      seq(kw("private"), kw("section"), ".", repeat($._section_statement)),
        ...
        repeat(
          choice(
            ...
            seq(
              ...
              ")",
              optional(
                seq(kw("type"), field("return_type", $._type_reference)),
              ),
            ),
    ...
    method_parameter: ($) =>
      seq(
        ...
        optional(
          seq(
            choice(kw("type"), kw("type ref to"), kw("like")),
    ...
    types_statement: ($) =>
      seq(kw("types"), optional(":"), sepBy1(",", $.type_item), "."),
    constants_statement: ($) =>
      seq(kw("constants"), optional(":"), sepBy1(",", $.data_item), "."),
        ...
        optional(
          seq(
            choice(
              ...
              kw("type ref to"),
              kw("type table of"),
              kw("type standard table of"),
              kw("type sorted table of"),
              kw("type hashed table of"),
              kw("like"),
              kw("like table of"),
            ),
            field("type", $._type_reference),
          ...
          seq(
            ...
            optional(
              seq(
                choice(
                  kw("into table"),
                  kw("into corresponding fields of table"),
                  kw("into"),
                  kw("into corresponding fields of"),
                ),
                field(
                  "target",
                  choice(
      ...
      choice(
        ...
        sepBy1(
          ...
          choice(
            $.identifier,
            seq($.identifier, "~", $.identifier),
            seq(kw("count"), "(", choice("*", $.identifier), ")"),
            seq(kw("sum"), "(", $.identifier, ")"),
            seq(kw("avg"), "(", $.identifier, ")"),
            seq(kw("min"), "(", $.identifier, ")"),
            seq(kw("max"), "(", $.identifier, ")"),
          ),
        ),
      ),
    ...
    loop_statement: ($) =>
      seq(
        kw("loop"),
        kw("at"),
        field("source", choice($.identifier, $._variable)),
        optional(
          choice(
            seq(
              kw("into"),
    ...
    read_table_statement: ($) =>
      seq(
        ...
        optional(
          seq(
            choice(kw("with key"), kw("with table key")),
            repeat1(
              choice(
                seq(
    ...
    when_statement_clause: ($) =>
      seq(
        kw("when"),
        sepBy1(choice(",", kw("or")), $._expression),
        ".",
        repeat($._statement),
      ),
    when_others_clause: ($) =>
      seq(kw("when others"), ".", repeat($._statement)),
    ...
    append_statement: ($) =>
      seq(
        ...
        optional(
          seq(kw("to"), field("target", choice($.identifier, $._variable))),
        ),
    ...
    modify_statement: ($) =>
      seq(
        ...
        choice(
          seq(
            field("target", choice($.identifier, $._variable)),
            optional(seq(kw("from"), field("source", $._expression))),
          ),
          seq(
            kw("table"),
            field("target", choice($.identifier, $._variable)),
    ...
    clear_statement: ($) =>
      seq(
        choice(kw("clear"), kw("free"), kw("refresh")),
        field("target", choice($.identifier, $._variable, $.field_symbol)),
        ".",
      ),
    write_statement: ($) =>
      seq(
        kw("write"),
        optional(":"),
        sepBy1(",", choice(seq(optional("/"), $._expression), "/")),
        ".",
      ),
    return_statement: ($) =>
      seq(
        choice(
          ...
          kw("exit"),
          kw("continue"),
          kw("leave to screen 0"),
          kw("check"),
          seq(kw("check"), $._expression),
        ),
        ".",
      ),
    raise_statement: ($) =>
    ...
    value_expression: ($) =>
      prec(
        ...
        seq(
          ...
          choice($.identifier, "#"),
          "(",
          repeat(choice($.value_row, $.value_field_assignment, $._expression)),
          ")",
        ),
      ),

    value_row: ($) =>
      seq("(", repeat(choice($.value_field_assignment, $._expression)), ")"),
    value_field_assignment: ($) =>
      seq(field("field", $.identifier), "=", field("val", $._expression)),
    ...
    corresponding_expression: ($) =>
      seq(
        ...
        optional(
          seq(kw("mapping"), repeat1(seq($.identifier, "=", $.identifier))),
        ),
        ")",
      ),
    ...
    reduce_expression: ($) =>
      seq(
        ...
        "(",
        kw("init"),
        field("init_var", $.identifier),
        "=",
        field("init_val", $._expression),
        kw("for"),
        field("for_item", $.identifier),
        kw("in"),
        field("for_source", $._expression),
        kw("next"),
        field("next_var", $.identifier),
        "=",
        field("next_val", $._expression),
        ")",
      ),
    ...
    unary_expression: ($) =>
      prec(4, seq(choice(kw("not"), "-", "+"), $._expression)),
