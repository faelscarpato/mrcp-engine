  ...
  rules: {
    ...
    bind_variable: ($) =>
      seq(
        ...
        choice(
          $.identifier,
          seq(choice(kw("NEW"), kw("OLD")), ".", $.identifier),
        ),
      ),
    string_literal: ($) => /'[^']*'/,
    number: ($) => /\d+(\.\d+)?([eE][+-]?\d+)?/,
          ...
          seq(
            ...
            sepBy1(
              ...
              seq(
                ...
                field("field_type", $._type_spec),
              ),
            ),
            ")",
          ),
          seq(
            choice(kw("TABLE"), kw("VARRAY")),
            optional(seq("(", $.number, ")")),
            kw("OF"),
            field("element_type", $._type_spec),
          ),
    ...
    exception_declaration: ($) =>
      seq(field("name", $.identifier), kw("EXCEPTION"), ";"),
    ...
    select_query: ($) =>
      seq(
        ...
        field(
          ...
          choice(
            "*",
            sepBy1(",", choice($.qualified_identifier, $.method_call)),
          ),
        ),
        optional(
          seq(
            choice(kw("INTO"), seq(kw("BULK"), kw("COLLECT"), kw("INTO"))),
            field(
              "into",
              sepBy1(",", choice($.qualified_identifier, $.bind_variable)),
            ),
          ),
        ),
        kw("FROM"),
        ...
        optional(
          seq(kw("GROUP"), kw("BY"), sepBy1(",", $.qualified_identifier)),
        ),
        optional(
          seq(kw("ORDER"), kw("BY"), sepBy1(",", $.qualified_identifier)),
        ),
    ...
    update_statement: ($) =>
      seq(
        ...
        sepBy1(
          ",",
          seq(
            field("column", $.qualified_identifier),
            "=",
            field("value", $._expression),
          ),
        ),
    ...
    cursor_close_statement: ($) =>
      seq(kw("CLOSE"), field("cursor", $.identifier), ";"),
    ...
    loop_statement: ($) =>
      seq(kw("LOOP"), repeat($._statement), kw("END"), kw("LOOP"), ";"),
    while_statement: ($) =>
      seq(
        kw("WHILE"),
        field("condition", $._expression),
    ...
    for_loop_statement: ($) =>
      seq(
        ...
        kw("IN"),
        choice(
          seq(field("low", $._expression), "..", field("high", $._expression)),
          field("cursor", $.qualified_identifier),
        ),
        kw("LOOP"),
        repeat($._statement),
        kw("END"),
    ...
    rollback_statement: ($) =>
      seq(
        kw("ROLLBACK"),
        optional(seq(kw("TO"), optional(kw("SAVEPOINT")), $.identifier)),
        ";",
      ),
    ...
    raise_statement: ($) =>
      seq(
        kw("RAISE"),
        optional(field("exception", $.qualified_identifier)),
        ";",
      ),
    return_statement: ($) =>
      seq(kw("RETURN"), optional(field("value", $._expression)), ";"),

    exception_block: ($) => seq(kw("EXCEPTION"), repeat1($.exception_handler)),
    ...
    unary_expression: ($) =>
      prec(3, seq(choice(kw("NOT"), "-", "+"), $._expression)),
