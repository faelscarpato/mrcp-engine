...
exports.packWorkspaceContextForAi = packWorkspaceContextForAi;
...
> *Recommendation: Reference and modify \`packages/core/\` as the canonical source.*\n\n---\n`;
    ...
    if (apiRoutes.length > 0) {
        ...
        for (const route of apiRoutes) {
            const aliasStr = route.aliases && route.aliases.length > 0
    ...
    for (const file of files.slice(0, 150)) {
        if (file.symbols.length === 0 && file.exports.length === 0)
            continue;
...
function packSingleFileContext(file, content) {
