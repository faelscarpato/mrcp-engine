...
function processSingleCodeFile(ctx) {
    ...
    const isTest = testFileMap.has(path.basename(filePath).toLowerCase()) ||
    ...
    while ((envMatch = envRegex.exec(content)) !== null) {
        ...
        const isCommentLine = lineContent.startsWith("//") ||
    ...
    const fileAvgComp = symbols.length > 0
        ? Math.max(1, fileFnComp /
