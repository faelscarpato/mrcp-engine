...
async function analyzeWorkspaceLocally(workspaceRoot, maxFiles = 2000, onProgress) {
    ...
    const allFilePaths = [];
    await (0, analyzer_helpers_1.scanDir)(workspaceRoot, workspaceRoot, allFilePaths, maxFiles);
    const codeFiles = allFilePaths.filter((f) => ast_extractors_1.CODE_EXTENSIONS.has(path.extname(f).toLowerCase()));
    const docFiles = allFilePaths.filter((f) => ast_extractors_1.DOC_EXTENSIONS.has(path.extname(f).toLowerCase()));
    ...
    for (let i = 0; i < codeFiles.length; i++) {
        ...
        onProgress?.(percent, `Analisando AST de ${path.basename(filePath)} (${i + 1}/${codeFiles.length})...`);
        try {
            const content = fileContentsMap.get(filePath) || fs.readFileSync(filePath, "utf8");
    ...
    docItems.push(...(0, analyzer_helpers_1.processDocumentFiles)(workspaceRoot, docFiles));
    ...
    const deadCodeItems = (0, analyzer_helpers_1.detectDeadCode)(workspaceRoot, allExportedSymbols, allImportedSymbolNames, fileContentsMap);
    ...
    const avgComplexity = totalFunctionCount > 0
        ...
        : 1.5;
    const maintainabilityIndex = totalFiles > 0 ? Math.round(totalFileMI / totalFiles) : 100;
    ...
    const docQualityAvg = docItems.length > 0
        ? Math.round(docItems.reduce((acc, d) => acc + d.qualityScore, 0) /
            docItems.length)
        : 100;
    ...
    const tokenSavingsPercent = rawTokens > 0
        ? Math.min(98, Math.max(75, Math.round(((rawTokens - mrcpTokens) / rawTokens) * 100)))
        : 95;
