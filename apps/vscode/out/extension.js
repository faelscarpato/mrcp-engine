...
function activate(context) {
    ...
    // 5. Register Commands
    context.subscriptions.push(vscode.commands.registerCommand("mrcp.startAutonomous", async () => {
        ...
        if (!prompt)
            return;
        const terminal = vscode.window.terminals.find((t) => t.name === "MRCP Autonomous") ||
            vscode.window.createTerminal("MRCP Autonomous");
        terminal.show();
        terminal.sendText(`npx mrcp-engine auto "${prompt}"`);
    }), vscode.commands.registerCommand("mrcp.runFullSuite", () => (0, run_suite_1.runFullSuiteCommand)(cmdContext)), vscode.commands.registerCommand("mrcp.openDashboard", () => {
