...
const SECRET_RULES = [
  { rule: 'AWS Key', regex: /(?:AKIA|ASIA)[0-9A-Z]{16}/, severity: 'critical' as const, msg: 'Hardcoded AWS Access Key detected.' },
  { rule: 'Generic Private Key', regex: /-----BEGIN (?:RSA|OPENSSH|EC|DSA) PRIVATE KEY-----/, severity: 'critical' as const, msg: 'Private cryptographic key exposed in source.' },
  { rule: 'GitHub Token', regex: /gh[pousr]_[0-9a-zA-Z]{36}/, severity: 'critical' as const, msg: 'GitHub Personal/OAuth Token detected.' },
  { rule: 'Hardcoded JWT', regex: /eyJ[a-zA-Z0-9_-]{10,}\.eyJ[a-zA-Z0-9_-]{10,}\.[a-zA-Z0-9_-]{10,}/, severity: 'high' as const, msg: 'Hardcoded JSON Web Token (JWT) detected.' },
  { rule: 'Dangerous Eval', regex: /(?<![\/\*a-zA-Z0-9_])eval\s*\([^)]+\)/, severity: 'high' as const, msg: 'Arbitrary code execution risk with eval().' },
  { rule: 'Hardcoded Password/Secret', regex: /(?:password|secret|apiKey|api_key|authToken)\s*[:=]\s*["'][a-zA-Z0-9_\-!@#$%^&*]{16,}["']/i, severity: 'medium' as const, msg: 'Possible hardcoded credential or secret.' }
];
...
export async function analyzeWorkspaceLocally(
  ...
  const allExportedSymbols = new Map<string, { file: string; line: number; kind: string; name: string }>();
    ...
    while ((impMatch = importBlockRegex.exec(content)) !== null) {
      if (impMatch[1]) {
        impMatch[1].split(',').forEach(s => {
          ...
          if (cleaned) allImportedSymbolNames.add(cleaned);
        });
      }
      if (impMatch[2]) allImportedSymbolNames.add(impMatch[2].trim());
      if (impMatch[3]) allImportedSymbolNames.add(impMatch[3].trim());
      if (impMatch[4]) {
  ...
  for (let i = 0; i < codeFiles.length; i++) {
    ...
    try {
      ...
      const exports: string[] = symbols.filter(s => s.isExported).map(s => s.name);
      ...
      for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
        ...
        if (!isSecurityScannerFile && !isTestFile && !trimmed.startsWith('//') && !trimmed.startsWith('/*') && !trimmed.startsWith('*')) {
          for (const sec of SECRET_RULES) {
            if (trimmed.includes('regex:') || trimmed.includes('RegExp(')) continue;
            if (sec.regex.test(line)) {
              if (line.includes('example') || line.includes('placeholder') || line.includes('mock') || line.includes('test')) continue;
              securityIssues.push({
 ...
 */
export function extractTypedAstSymbols(
  content: string,
  filePath: string,
  ext: string,
  isTestFile: boolean,
...
function parseTypedParameters(raw: string): Array<{ name: string; type?: string; optional?: boolean }> {
  if (!raw.trim()) return [];
  const parts: string[] = [];
  let current = '';
  let depth = 0;
...
function calculateFunctionBodyComplexity(content: string, startIndex: number): number {
  let openBraces = 1;
 ...
 */
function extractApiRoutes(workspaceRoot: string, codeFiles: string[], apiRoutes: MrcpApiRoute[]): void {
 ...
 */
function detectDuplicateModules(workspaceRoot: string): MrcpDuplicateModule[] {
...
async function scanDir(root: string, currentDir: string, result: string[], maxFiles: number): Promise<void> {
...
function resolveImportPaths(file: string, rawImports: string[], allFiles: string[], root: string): string[] {
...
function findCycles(graph: Map<string, string[]>): MrcpDependencyCycle[] {
...
function getLanguageName(ext: string): string {
  switch (ext) {
    ...
    case '.c': case '.cpp': return 'C/C++';
    case '.php': return 'PHP';
    case '.rb': return 'Ruby';
    case '.cs': return 'C#';
    case '.cds': return 'SAP CDS';
    case '.abap': case '.clas': case '.intf': case '.prog': return 'SAP ABAP';
    case '.sql': case '.pls': case '.pks': case '.pkb': case '.pck': case '.plb': case '.trg': case '.fnc': case '.prc': return 'Oracle PL/SQL';
    case '.pas': case '.pp': case '.inc': return 'Pascal';
    case '.cbl': case '.cob': case '.cpy': return 'COBOL';
