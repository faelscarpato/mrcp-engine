...
export async function analyzeWorkspaceLocally(
  ...
  const allExportedSymbols = new Map<string, { name: string; file: string; line: number; kind: string }>();
  ...
  const deadCodeItems = detectDeadCode(workspaceRoot, allExportedSymbols, allImportedSymbolNames, fileContentsMap);
