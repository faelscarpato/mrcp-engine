...
import { copyAiContextCommand, copyFileContextCommand } from './commands/copy-context';
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const cmdContext: CommandContext = {
    ...
    setLastResult: (res: MrcpSuiteResult) => { lastSuiteResult = res; }
  };

  // 5. Register Commands
  context.subscriptions.push(
    vscode.commands.registerCommand('mrcp.runFullSuite', () => runFullSuiteCommand(cmdContext)),

    vscode.commands.registerCommand('mrcp.openDashboard', () => {
      MrcpDashboardPanel.createOrShow(context.extensionUri, lastSuiteResult);
    }),

    vscode.commands.registerCommand('mrcp.copyAiContext', () => copyAiContextCommand(cmdContext)),

    vscode.commands.registerCommand('mrcp.copyFileContext', () => copyFileContextCommand(cmdContext)),
    ...
    vscode.commands.registerCommand('mrcp.exportReport', () => exportReportCommand(cmdContext)),

    vscode.commands.registerCommand('mrcp.refresh', () => runFullSuiteCommand(cmdContext)),

    vscode.commands.registerCommand('mrcp.openFileAtLocation', async (filePath: string, line: number) => {
      try {
