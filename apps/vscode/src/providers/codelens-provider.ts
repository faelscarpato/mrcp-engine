...
export class MrcpCodeLensProvider implements vscode.CodeLensProvider {
  ...
  provideCodeLenses(document: vscode.TextDocument, token: vscode.CancellationToken): vscode.CodeLens[] | Thenable<vscode.CodeLens[]> {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (fnMatch && fnMatch[1] && !['if', 'for', 'while', 'switch'].includes(fnMatch[1])) {
        const symName = fnMatch[1];
        const range = new vscode.Range(i, 0, i, line.length);

        // Approximate complexity
        const complexityTokens = line.match(/\b(if|for|while|case|catch)\b|\?|&&|\|\|/g);
        const comp = (complexityTokens?.length || 0) + 1;
        const compLabel = comp > 10 ? 'Alta 🔴' : comp > 5 ? 'Média 🟡' : 'Baixa 🟢';
